...
export class VRMAnimationLoaderPlugin implements GLTFLoaderPlugin {
  ...
  public async afterRoot(_gltf: GLTF): Promise<void> {
    const gltf = _gltf as VRMGLTF;
    ...
    const defExtension = defGltf.extensions?.[this.name] as VRMCVRMAnimation | null;
      ...
      if (boneName != null) {
        ...
        if (path === "translation") {
          ...
          const trackValues = arrayChunk(origTrack.values, 4).flatMap((q) =>
            _quatC.fromArray(q as THREE.QuaternionTuple).premultiply(_quatB).multiply(_quatA).toArray()
          );
